// Dual Air & Soil Telemetry Studio Engine (BME280 + AHT20 + DS18B20 + Moisture)
...
function applyTimeFilter() {
  ...
  if (activeRangeMode === "1h") {
    ...
    elFilterSummaryText.textContent = summaryText;
    updateDashboardUI();
    return;
  }
...
async function fetchTelemetryData() {
  ...
  try {
    ...
    if (data && (data.Live || data.History)) {
      setConnectionOnline(true, "Firebase Realtime DB Online");
    ...
    setConnectionOnline(false, "Waiting for ESP32 Firebase Stream...");
  }
}
...
async function fetchAhtFallbackData() {
  ...
  setConnectionOnline(false, "Offline (Click 'Sim Telemetry' to test)");
}

function setConnectionOnline(online, message) {
  isConnected = online;
  elStatusText.textContent = message;
  if (online) elStatusBadge.classList.add("online");
  else elStatusBadge.classList.remove("online");
}
...
function updateDashboardUI() {
  ...
  elBmeTemp.innerHTML = `${formatNum(bmeTemp, 1)} <span class="unit">°C</span>`;
  elAhtTemp.innerHTML = `${formatNum(ahtTemp, 1)} <span class="unit">°C</span>`;
  elSoilTemp.innerHTML = `${formatNum(latest.SoilTemp_C, 1)} <span class="unit">°C</span>`;
  elBmeHum.innerHTML = `${formatNum(bmeHum, 1)} <span class="unit">%</span>`;
  elAhtHum.innerHTML = `${formatNum(ahtHum, 1)} <span class="unit">%</span>`;
  elCapMoist.innerHTML = `${formatNum(latest.CapMoisture_Pct, 0)} <span class="unit">%</span>`;
  elResMoist.innerHTML = `${formatNum(latest.ResMoisture_Pct, 0)} <span class="unit">%</span>`;
  elCapRaw.textContent = latest.CapMoisture_Raw ?? "--";
  elResRaw.textContent = latest.ResMoisture_Raw ?? "--";
  elBmePress.innerHTML = `${formatNum(latest.BME_AirPressure_hPa ?? latest.AirPressure_hPa, 1)} <span class="unit">hPa</span>`;

  // 2. Update Charts with Filtered Data Window
  updateCharts(filteredHistory);

  // 3. Update Table
  updateTable(filteredHistory);

  elRecordsCount.textContent = `${filteredHistory.length} Records Filtered (${rawTelemetryHistory.length} Total)`;
  elLastUpdate.textContent = `Last update: ${latest.Time || latest.Timestamp || new Date().toLocaleTimeString()}`;
}

function updateCharts(records) {
  // Downsample to max 50 points on chart for crisp rendering
...
function updateTable(records) {
  ...
  reversed.forEach(r => {
    ...
    tr.innerHTML = `
      ...
      <td>${formatNum(r.BME_AirTemp_C ?? r.AirTemp_C, 1)}</td>
      <td>${formatNum(r.AHT_AirTemp_C ?? r.AirTemp_C, 1)}</td>
      <td>${formatNum(r.SoilTemp_C, 1)}</td>
...
function exportFilteredCsvLog() {
  ...
  const headers = [
    ...
    "CapMoisture_Raw", "ResMoisture_Raw", "CapMoisture_Pct", "ResMoisture_Pct"
  ];
...
function toggleDemoSimulation() {
  if (isDemoActive) {
    ...
    elBtnDemo.innerHTML = `<i class="fa-solid fa-wand-magic-sparkles"></i> Sim Telemetry`;
  ...
  elBtnDemo.innerHTML = `<i class="fa-solid fa-stop"></i> Stop Sim`;
